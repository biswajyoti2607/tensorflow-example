...
__version__ = '0.1'
__author__ = 'Biswajyoti Pal (biswajyoti2607@gmail.com, biswajyoti@gatech.edu)'
...
data_type_dirs = {'train': 'train', 'test': 'test'}
model_dir = "model"
...
def preprocess(data_type):
    for file in listdir(data_dir + "/" + data_type_dirs[data_type]):
        if file.endswith(".jpg"):
            orig_image = Image.open(data_dir + "/" + data_type_dirs[data_type]
                                    + "/" + file)
            resized_image = orig_image.resize((200, 200), Image.ANTIALIAS)
            resized_image.save(
                preprocessed_data_dir + "/" + data_type_dirs[data_type] + "/" +
                file,
                'JPEG',
                quality=90)
...
def load_data(data_type):
    data = []
    labels = []
    for file in listdir(preprocessed_data_dir + "/" +
                        data_type_dirs[data_type]):
        if file.endswith(".jpg"):
            image = Image.open(preprocessed_data_dir + "/" +
                               data_type_dirs[data_type] + "/" + file)
...
def generate_model():
    ...
    model = tflearn.DNN(
        convnet, tensorboard_verbose=3, tensorboard_dir=vis_dir)
    return model
...
def train(run=100):
    ...
    model.fit(
        ...
        run_id=model_name)
    endTs = time.time()
    logger.info("Training with run=" + str(run) + " runtime=" +
                str(endTs - startTs))
